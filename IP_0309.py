...
def read_onboard_passenger(vehicle_list,iteration=0):
    #读取格式[index,vehicle_index,d_time,d_station] 0314更新 不使用保存的index
    ...
    for passenger in passenger_information:
        start_pos=vehicle_list[passenger[1]].start_pos
        new=Passenger(passenger_index,0,start_pos,passenger[2],passenger[3])
        passenger_list.append(new)
        core_boarded_passenger_list.append(new)
        new.board_bus=passenger[1]
        passenger_index+=1
        time_horizon=max(time_horizon,passenger[2])
...
def read_new_passenger(iteration=0):
    ...
    passenger_information=np.load('experiment_%d/new_passenger_%d.npy'% (experiment,iteration))
    passenger_list=[]
    for passenger in passenger_information:
...
def get_passenger_area(passenger,net,type):#type 1 AO type 2 AD type 3 AOUAD
    o=passenger.o_station_index
    ot=passenger.o_time_index
    d=passenger.d_station_index
    dt=passenger.d_time_index
    if type==1:
        return [net['N'][i][o] for i in range(ot,dt)]
    elif type==2:
        return [net['N'][i][d] for i in range(ot,dt)]
    elif type==3:
        return [net['N'][i][o] for i in range(ot,dt)]+[net['N'][j][d] for j in range(ot,dt)]
    else:
        return []
...
for p in core_new_passenger_list:
    for i in get_passenger_area(p,net,3):
        ...
        b=lp.lpSum([var_y[p.index][q.index][i.node_index][k.node_index] for q in vehicle[1:] for k in i.out_node[:-1]])
        c=lp.lpSum([var_y[p.index][q.index][i.node_index][k.node_index] for q in vehicle[1:] for k in i.out_node])
        d=lp.lpSum([var_y[p.index][q.index][g.node_index][i.node_index] for q in vehicle[1:] for g in i.in_node])
        prob+= a+var_y[p.index][0][i.in_node[-1].node_index][i.node_index]-var_y[p.index][0][i.node_index][i.out_node[-1].node_index]-b == 0,''
        prob+= var_y[p.index][0][i.node_index][i.out_node[-1].node_index]+c <= 1,''
        prob+= var_y[p.index][0][i.in_node[-1].node_index][i.node_index]+d <= 1,''
...
for p in core_boarded_passenger_list:
    i=net['FULL'][p.o_time_index][p.o_station_index]
    a=lp.lpSum([var_y[p.index][q.index][i.node_index][j.node_index] for q in [k for k in vehicle[1:] if (k.index!=p.board_bus)] for j in i.out_node])
    prob+= a == 0,''
#constraint 10
for p in core_new_passenger_list:
    ...
    b=lp.lpSum([var_y[p.index][q.index][i.node_index][j.node_index] for q in vehicle[1:] for j in i.out_node])
    prob+= a + var_y[p.index][0][i.in_node[-1].node_index][i.node_index] + get_dummy_var(var_y,p,net) == 1,''
    prob+= b + var_y[p.index][0][i.node_index][i.out_node[-1].node_index]== 0,''
#constraint 11
for p in core_boarded_passenger_list:
    i=net['FULL'][p.d_time_index][p.d_station_index]
    a=lp.lpSum([var_y[p.index][q.index][h.node_index][i.node_index] for q in vehicle[1:] for h in i.in_node])
...
#     for p in core_passenger_list:
#         for ij in get_all_legal_arc(net):
#             a=lp.lpSum([var_y[p.index][q.index][ij[0].node_index][ij[1].node_index] for q in vehicle ])
#             prob+= a ==1,''
