...
for i in range(1,max_vehicle+1):
    ...
    while iteration<=max_iteration:
        ...
        #constraint 13
        for q in vehicle[1:]:
            for i in core_node_list:
                related_passenger=get_related_passenger(i)
                if len(related_passenger)>0:
                    for j in i.out_node:
                        a=lp.lpSum([var_y(p.index,q.index,i.node_index,j.node_index) for p in get_related_passenger(i)])
                        prob+= var_x(q.index,i.node_index,j.node_index) - a >=0,''
        # for q in vehicle[1:]:
        #     for p in core_passenger_list:
        ...
        #     for i in core_node_list:
        #         for j in i.out_node:
        #             a=lp.lpSum([var_y(p.index,q.index,i.node_index,j.node_index) for p in core_passenger_list])
        #             prob+= var_x(q.index,i.node_index,j.node_index) - a >=0,''
